...
export function ShaderAnimation() {
    ...
    `;
        ...
        // Fragment shader - Themed to Gold
      ...
      void main(void) {
        ...
        // Gold Base: #D4AF37 (0.83, 0.68, 0.21)
        vec3 gold = vec3(0.83, 0.68, 0.21);
        vec3 color = vec3(0.0);
        
        for(int j = 0; j < 3; j++){
          for(int i=0; i < 5; i++){
            // Calculate intensity with slight offsets for depth
            float intensity = lineWidth * float(i*i) / abs(fract(t - 0.01 * float(j) + float(i) * 0.01) * 5.0 - length(uv) + mod(uv.x + uv.y, 0.2));
            
            // Apply gold tint instead of RGB
            color += gold * intensity;
          }
        }
        
        // Ensure some alpha for blending if used as a layer
